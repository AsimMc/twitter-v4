        ...
        <div className="flex items-center justify-between">
            ...
            <div className="flex justify-between text-gray-500 p-2">
              <div className="flex items-center select-none">
                    ...
                    setPostId(post.id); setOpen(!open);}} className="h-9 w-9 hoverEffect p-2 hover:text-red-500 hover:bg-red-100"/>
                    </div>
                    {session?.user.uid === comment?.userId && (

                    
                    <TrashIcon onClick={deleteComment} className="h-9 w-9 hoverEffect p-2 hover:text-red-500 hover:bg-red-100"/>
                    )}
                    <div className="flex items-center">

                    {hasLiked ? (
                    <HeartIconFilled onClick={likeComment} className="h-9 w-9 hoverEffect p-2 text-red-500 hover:bg-red-100"/>) :(
                    <HeartIcon onClick={likeComment} className="h-9 w-9 hoverEffect p-2 hover:text-red-500 hover:bg-red-100"/>)}
                    {
                      likes.length > 0 && (
                        <span className={`${hasLiked && "text-red-600"} select-none`}>{likes.length}</span>
                      )
                    }
                    </div>
                    <ShareIcon className="h-9 w-9 hoverEffect p-2 hover:text-red-500 hover:bg-red-100"/>
                    <ChartBarIcon className="h-9 w-9 hoverEffect p-2 hover:text-red-500 hover:bg-red-100"/>
            </div>
        </div>
    </div>
  )

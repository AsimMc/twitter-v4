...
export default function Input() {
  ...
  return (
    ...
    <div className="flex  border-b border-gray-200 p-3 space-x-3">
        <img onClick={signOut} src={session?.user?.image} alt="user-img" className="h-11 w-11 rounded-full cursor-pointer hover:brightness-95" />
        <div className="w-full divide-y divide-gray-200">
            ...
            <div className="flex items-center justify-between pt-2.5">
                ...
                <button onClick={sendPost} disabled={!input.trim()} className="bg-rose-600 text-white px-4 py-1.5 rounded-full font-bold shadow-md hover:brightness-95 disabled:opacity-50">Post</button>
                </>
                )}
            </div>
        </div>
